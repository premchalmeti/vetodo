...
export class TodoComponent implements OnInit {
  ...
  constructor(private todoService: TodoService, public todoEditDialog: MatDialog) { }
  ...
  openEditTodoDialog(): void {
    ...
    todoDialog.afterClosed().subscribe(
      (data) => {
        if(!data) return;
  ...
  openTodoReminderDialog(): void {
    const reminderDialog = this.todoEditDialog.open(DatetimePickerDialogComponent, {
      width: '500px',
      data: {}
    })

    reminderDialog.afterClosed().subscribe(
      (reminderDateTimeMomentObject) => {
        if(!reminderDateTimeMomentObject) return;
        reminderDateTimeMomentObject.seconds(0);
        console.log(`Reminder Added: ${JSON.stringify(reminderDateTimeMomentObject)}`);
  ...
  update(updatedTodo: Todo) {
    this.todoService.updateTodo(updatedTodo).subscribe((response) => {
      if(response.ok) {
  ...
  delete() {
    ...
    this.todoService.deleteTodo(this.todo).subscribe((response) => {
  ...
  addReminder(reminderDateTime: Date) {
    this.todoService.addReminder(this.todo, reminderDateTime).subscribe(
      (response) => {
        if(response.ok) {
